...
def get_temperature_jour(annee, mois, jour):
    ...
    return render_template("affichage_temperature_jour.html", nom_image_min=nommer_jour(MITJ, annee, mois, jour), nom_image_max=nommer_jour(MATJ, annee, mois),
                           nom_image_moyenne=nommer_jour(MOTJ, annee), annee=annee, mois=l_mois[mois-1], jour=jour)
...
def get_data_global_jour(annee, mois, jour):
    nom_fichier_json = nommer_jour_json("data_jour_", annee, mois, jour)
    ...
    with open(nom_fichier_json, "rb") as f:
